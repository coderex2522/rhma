...
static void rhma_wc_success_handler ( struct ibv_wc* wc )
{
	...
	rdma_trans= ( struct rhma_transport* ) ( uintptr_t ) wc->wr_id;


	switch ( wc->opcode )
	{
		case IBV_WC_SEND:
			INFO_LOG("send content is %d",rdma_trans->node_id);
			break;
		case IBV_WC_RECV:
			INFO_LOG("recv content is %d",rdma_trans->node_id);
			break;
		case IBV_WC_RDMA_WRITE:
			break;
		case IBV_WC_RDMA_READ:
...
static int on_cm_established ( struct rdma_cm_event* event, struct rhma_transport* rdma_trans )
{
	...
	if(rdma_trans->node_id==0)
		rhma_post_send(rdma_trans, NULL);
...
int rhma_post_recv ( struct rhma_transport* rdma_trans )
{
	...
	sge.length=RECV_REGION_SIZE;
	sge.lkey=rdma_trans->recv_region_mr->lkey;
...
int rhma_post_send(struct rhma_transport *rdma_trans, struct rhma_msg *msg)
{
	...
	snprintf(rdma_trans->send_region, SEND_REGION_SIZE, "message from active/client side with pid");
	...
	sge.length=SEND_REGION_SIZE;
	sge.lkey=rdma_trans->send_region_mr->lkey;
